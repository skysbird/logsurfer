	...
	struct rule	*rule_ptr;
{
	...
	if(rule_ptr->state !=NULL){
		free(rule_ptr->state);
	}

	if(rule_ptr->next_state !=NULL){
		free(rule_ptr->next_state);
	}
	...
	char	*input_line;
{
	...
	/* get the match_regex */
	help_ptr=input_line;
	if ( (new_rule->state=get_word(&help_ptr)) == NULL ) {
		(void) fprintf(stderr, "error in state of rule: %s\n", input_line);
	...
	help_ptr=skip_spaces(help_ptr);
	if ( *help_ptr != '-' ) {
		if ( (new_rule->next_state=get_word(&help_ptr)) == NULL ) {
			(void) fprintf(stderr, "error in next_state of rule: %s\n", input_line);
			destroy_rule(new_rule);
			return(NULL);
		}
