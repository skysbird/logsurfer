...
char		tag_str[MAXPATHLEN+50]; /* tag will come up in report */
...
char *state = NULL; /* global state machine status */
...
(void) printf("processing logline: %s\n", logline);
	...
	for ( this_rule=all_rules; this_rule != NULL ; ) {
		/* check global state and if the logline matches the first rgex */
		if ( strcmp(state,this_rule->state)==0 && re_search(this_rule->match_regex, logline, strlen(logline),
			0, strlen(logline), &regex_submatches) >= 0 ) {
	...
	char	**argv;
{
	...
	if ( argc == 0 || !strcmp(argv[0], "-") ) {
		...
		sprintf(tag_str,"from logfile: %s",logfile_name);
