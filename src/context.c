...
(void) printf("opening new context: ***%s***\n", context_def);
	...
	char* w = get_word(&src);
	//printf("match_regex_str is %s\n",w);
	if ( (new_context->match_regex_str=w) == NULL ) {
...
(void) printf("context already exists: ***%s***\n", new_context->match_regex_str);
	...
	if(*src!='-'){
		if ( (new_context->next_state=get_word(&src)) == NULL ) {
				(void) fprintf(stderr, "error in next_state creating context: %s\n",
					context_def);
				destroy_context(new_context);
				return;
			}
...
//			printf("context_ptr is %s\n",context_ptr->match_regex_str);
//			printf("compaire_regex is %s\n",compare_regex);
